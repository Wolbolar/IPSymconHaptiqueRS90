...
class HaptiqueMacro extends IPSModule
{
    public function Create()
    {
        ...
        // Macro
        $this->RegisterVariableBoolean('Trigger', $this->Translate('Trigger Macro'), '~Switch', 1);
        $this->EnableAction('Trigger'); // Make switchable
    ...
    private function FillHTMLAndScriptTemplate($dynamicContent, $scriptFileName)
    {
    ...
    // Public methods for direct script calls
     ...
     * @return void
     */
    public function TriggerMacro(bool $Value)
    {
        $remoteId = $this->ReadPropertyString('RemoteID');
        $MacroName = $this->ReadPropertyString('MacroName');
        $topic   = sprintf('Haptique/%s/macro/%s/trigger', $remoteId, $MacroName);
        if ($Value) {
           $command = "on";
        }
        else {
        ...
        $this->SendDebug('Macro MQTT Topic', $topic, 0);
        $this->SendDebug('Macro MQTT Payload', $payload, 0);
    ...
    public function GetConfigurationForm()
    {
        $form = [
            'elements' => [
                ...
                [
                    'type' => 'Label',
                    'caption' => $this->Translate('Haptique Macro Configuration')
                ],
                [
                    'type' => 'ValidationTextBox',
                    'name' => 'MacroName',
                    'caption' => $this->Translate('Macro Name'),
                    'value' => $this->ReadPropertyString('MacroName'),
                    'enabled' => true
                ],
                [
                    'type' => 'ValidationTextBox',
                    ...
                    'value' => $this->ReadPropertyString('MacroID'),
                    'enabled' => true
                ],
                [
                    'type' => 'CheckBox',
            ...
            'actions' => [
                [
                    'type' => 'RowLayout',
                    'items' => [
                        ['type' => 'Button', 'caption' => $this->Translate('Macro on'), 'onClick' => 'CRSX_TriggerMacro($id, true);'],
                        ['type' => 'Button', 'caption' => $this->Translate('Macro off'), 'onClick' => 'CRSX_TriggerMacro($id, false);']
                    ]
                ]
            ],
            'status' => [
