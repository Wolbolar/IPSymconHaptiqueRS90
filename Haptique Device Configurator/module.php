<?php

declare(strict_types=1);

class HaptiqueDeviceConfigurator extends IPSModuleStrict
{
    public function GetCompatibleParents(): string
    {
        return json_encode([
            'type' => 'connect',
            'moduleIDs' => ['{F7A0DD2E-7684-95C0-64C2-D2A9DC47577B}'] // Parent: MQTT Client (gleich wie im Configurator)
        ]);
    }
    public function Create(): void
    {
        //Never delete this line!
        parent::Create();


        $this->RegisterAttributeString('HaptiqueDevices', "[]");
        // Per-Remote storage (multiple RS90 in one network)
        $this->RegisterAttributeString('RemoteIDs', '[]');                 // JSON array of discovered remote IDs
        $this->RegisterAttributeString('DeviceListsByRemote', '{}');       // JSON object: { remoteId: [devices...] }
        $this->RegisterAttributeString('MacroListsByRemote', '{}');        // JSON object: { remoteId: [macros...] }
        $this->RegisterAttributeString('LastKeyByRemote', '{}');           // JSON object: { remoteId: "button:10" }
        $this->RegisterAttributeString('BatteryByRemote', '{}');           // JSON object: { remoteId: 87 }
    }

    public function Destroy(): void
    {
        //Never delete this line!
        parent::Destroy();
    }

    public function ApplyChanges(): void
    {
        //Never delete this line!
        parent::ApplyChanges();

        // Nur Pakete mit Topic "Haptique/..." durchlassen
        $this->SetReceiveDataFilter('.*"Topic"\s*:\s*"Haptique\\/.*".*');
    }

    private function GetJsonAttr(string $name, $default)
    {
        $raw = $this->ReadAttributeString($name);
        $val = json_decode($raw, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            return $default;
        }
        return $val;
    }

    private function SetJsonAttr(string $name, $value): void
    {
        $this->WriteAttributeString($name, json_encode($value));
    }

    private function EnsureRemoteId(string $remoteId): void
    {
        if ($remoteId === '') {
            return;
        }
        $ids = $this->GetJsonAttr('RemoteIDs', []);
        if (!is_array($ids)) {
            $ids = [];
        }
        if (!in_array($remoteId, $ids, true)) {
            $ids[] = $remoteId;
            sort($ids);
            $this->SetJsonAttr('RemoteIDs', $ids);
        }
    }

    private function SetMapValue(string $attr, string $remoteId, $value): void
    {
        if ($remoteId === '') {
            return;
        }
        $map = $this->GetJsonAttr($attr, []);
        if (!is_array($map)) {
            $map = [];
        }
        $map[$remoteId] = $value;
        $this->SetJsonAttr($attr, $map);
    }

    // ===== Public getters for scripts =====

    public function GetRemoteIDs(): array
    {
        return $this->GetJsonAttr('RemoteIDs', []);
    }

    public function GetDeviceListsByRemote(): array
    {
        return $this->GetJsonAttr('DeviceListsByRemote', []);
    }

    public function GetMacroListsByRemote(): array
    {
        return $this->GetJsonAttr('MacroListsByRemote', []);
    }

    public function GetLastKeyByRemote(): array
    {
        return $this->GetJsonAttr('LastKeyByRemote', []);
    }

    public function GetBatteryByRemote(): array
    {
        return $this->GetJsonAttr('BatteryByRemote', []);
    }

    // Convenience helpers
    public function GetDevicesForRemote(string $remoteId): array
    {
        $map = $this->GetJsonAttr('DeviceListsByRemote', []);
        return $map[$remoteId] ?? [];
    }

    public function GetMacrosForRemote(string $remoteId): array
    {
        $map = $this->GetJsonAttr('MacroListsByRemote', []);
        return $map[$remoteId] ?? [];
    }

    public function GetLastKeyForRemote(string $remoteId): string
    {
        $map = $this->GetJsonAttr('LastKeyByRemote', []);
        return (string) ($map[$remoteId] ?? '');
    }

    public function GetBatteryForRemote(string $remoteId): int
    {
        $map = $this->GetJsonAttr('BatteryByRemote', []);
        return (int) ($map[$remoteId] ?? -1);
    }

    private function SendToSplitter(array $payload)
    {
        // Anfrage an den Splitter senden
        $response = $this->SendDataToParent(json_encode([
            'DataID' => '{043EA491-0325-4ADD-8FC2-A30C8EEB4D3F}', // Hier die richtige DataID einsetzen
            'Payload' => $payload
        ]));

        // Antwort decodieren und zurückgeben
        return json_decode($response, true);
    }

    // ===== MQTT publish helpers (TX to MQTT Client) =====

    /**
     * Publish a MQTT message via the connected MQTT Client instance (parent).
     * PacketType 3 = PUBLISH (default).
     */
    public function Publish(string $topic, string $payload = '', int $qos = 0, bool $retain = false, int $packetType = 3): void
    {
        $topic = trim($topic);
        if ($topic === '') {
            return;
        }

        // DataID for TX to MQTT Client
        $data = [
            'DataID'           => '{043EA491-0325-4ADD-8FC2-A30C8EEB4D3F}',
            'PacketType'       => $packetType,
            'QualityOfService' => $qos,
            'Retain'           => $retain,
            'Topic'            => $topic,
            'Payload'          => $payload
        ];

        $this->SendDebug('Publish', json_encode($data), 0);
        $this->SendDataToParent(json_encode($data));
    }

    /**
     * Convenience: request battery status from a given remote.
     * According to Haptique MQTT API, publishing to the battery/status topic triggers a response.
     */
    public function RequestBatteryStatus(string $remoteId): void
    {
        $remoteId = trim($remoteId);
        if ($remoteId === '') {
            return;
        }
        $this->Publish('Haptique/' . $remoteId . '/battery/status', '', 0, false, 3);
    }

    /**
     * Convenience: request key events stream (if the remote only starts sending after a publish).
     */
    public function RequestKeyEvents(string $remoteId): void
    {
        $remoteId = trim($remoteId);
        if ($remoteId === '') {
            return;
        }
        $this->Publish('Haptique/' . $remoteId . '/keys', '', 0, false, 3);
    }

    /**
     * Request (subscribe to) device list for a given RS90 remote.
     * Topic: Haptique/{RemoteID}/device/list
     */
    public function RequestDeviceList(string $remoteId): void
    {
        $remoteId = trim($remoteId);
        if ($remoteId === '') {
            return;
        }

        $topic = 'Haptique/' . $remoteId . '/device/list';

        // Subscribe packet via MQTT Client
        $data = [
            'DataID'           => '{043EA491-0325-4ADD-8FC2-A30C8EEB4D3F}',
            'PacketType'       => 8, // SUBSCRIBE
            'QualityOfService' => 0,
            'Retain'           => false,
            'Topic'            => $topic,
            'Payload'          => ''
        ];

        $this->SendDebug('Subscribe DeviceList', json_encode($data), 0);
        $this->SendDataToParent(json_encode($data));
    }

    /**
     * Request (subscribe to) macro list for a given RS90 remote.
     * Topic: Haptique/{RemoteID}/macro/list
     */
    public function RequestMacroList(string $remoteId): void
    {
        $remoteId = trim($remoteId);
        if ($remoteId === '') {
            return;
        }

        $topic = 'Haptique/' . $remoteId . '/macro/list';

        // Subscribe packet via MQTT Client
        $data = [
            'DataID'           => '{043EA491-0325-4ADD-8FC2-A30C8EEB4D3F}',
            'PacketType'       => 8, // SUBSCRIBE
            'QualityOfService' => 0,
            'Retain'           => false,
            'Topic'            => $topic,
            'Payload'          => ''
        ];

        $this->SendDebug('Subscribe MacroList', json_encode($data), 0);
        $this->SendDataToParent(json_encode($data));
    }

    protected function FindExistingInstances(string $guid, string $property): array
    {
        $instances = [];
        foreach (IPS_GetInstanceList() as $id) {
            $inst = IPS_GetInstance($id);
            if ($inst['ModuleInfo']['ModuleID'] === $guid) {
                $ID = @IPS_GetProperty($id, $property);
                if ($ID) {
                    $instances[$ID] = $id;
                }
            }
        }
        // $debug_text = json_encode($instances);
        // $this->SendDebug(__FUNCTION__, "✅ Bereits verlinkte Instanzen gefunden: $debug_text", 0);
        return $instances;
    }

    private function LoadRSDevices(): array
    {
        $remoteIds = $this->GetJsonAttr('RemoteIDs', []);
        if (!is_array($remoteIds)) {
            $remoteIds = [];
        }

        // Vorhandene RS90 Remote Instanzen ermitteln
        $existingInstances = $this->FindExistingInstances("{475D0935-C17C-F98D-558B-0268B75FA78B}", "RemoteID");

        $batteryMap = $this->GetJsonAttr('BatteryByRemote', []);
        $remoteRows = [];
        foreach ($remoteIds as $rid) {
            $rid = (string)$rid;
            $instanceID = 0;
            if (isset($existingInstances[$rid])) {
                $instanceID = $existingInstances[$rid];
                $this->SendDebug(__FUNCTION__, "✅ Bereits verlinkte Instanz gefunden: $instanceID für RemoteID $rid", 0);
            }

            $remoteRows[] = [
                'instanceID' => $instanceID,
                'RemoteID' => $rid,
                'Model' => "RS90",
                'Battery'  => $batteryMap[$rid] ?? -1,
                'create' => [
                    "caption" => "RS90 Remote Instanz erstellen",
                    "moduleID" => "{475D0935-C17C-F98D-558B-0268B75FA78B}",
                    "location" => [],
                    "name" => "RS90_" . $rid,
                    "configuration" => [
                        "RemoteName" => "RS90_" . $rid ?? 'Unbekannt',
                        "RemoteModel" => "RS90" ?? 'Unbekannt',
                        "RemoteID" => $rid ?? 'Unbekannt'
                    ]
                ]
            ];
        }
        return $remoteRows;
    }

    private function TreeId(string $key): int
    {
        // Stable positive 31-bit integer (1..2147483647) for Symcon Tree
        $id = (int) hexdec(substr(md5($key), 0, 8));

        // Reduce to signed 31-bit range
        $id = $id & 0x7FFFFFFF;

        if ($id === 0) {
            $id = 1;
        }
        return $id;
    }

    private function LoadDevices(): array
    {
        $remoteIds = $this->GetJsonAttr('RemoteIDs', []);
        if (!is_array($remoteIds)) {
            $remoteIds = [];
        }

        // Vorhandene RS90 Device Instanzen ermitteln
        $existingInstances = $this->FindExistingInstances("{3AD62E8A-68C2-BA18-223D-EF022958AE42}", "DeviceID");

        $devMap     = $this->GetJsonAttr('DeviceListsByRemote', []);
        if (!is_array($devMap))     $devMap = [];


        $deviceRows = [];
        foreach ($remoteIds as $rid) {
            $rid = (string)$rid;

            $remoteNodeId = $this->TreeId('remote:' . $rid);

            // Parent row (Remote)
            $deviceRows[] = [
                'id'     => $remoteNodeId,
                'parent' => 0,
                'Node'   => $rid,
                'DeviceID'     => '',
                'DeviceName'   => ''
            ];

            $devices = $devMap[$rid] ?? [];
            if (is_array($devices)) {
                foreach ($devices as $d) {
                    if (!is_array($d)) continue;
                    $did  = (string)($d['id'] ?? '');
                    $name = (string)($d['name'] ?? '');
                    // $this->SendDebug(__FUNCTION__, "Gerät $name mit DeviceID $did", 0);
                    $instanceID = 0;
                    if (isset($existingInstances[$did])) {
                        $instanceID = $existingInstances[$did];
                        $this->SendDebug(__FUNCTION__, "✅ Bereits verlinkte Instanz gefunden: $instanceID für DeviceID $did", 0);
                    }

                    $childId = $this->TreeId('device:' . $rid . ':' . $did);

                    $deviceRows[] = [
                        'instanceID' => $instanceID,
                        'id'     => $childId,
                        'parent' => $remoteNodeId,
                        'Node'   => '',
                        'DeviceID'     => $did,
                        'DeviceName'   => $name,
                        'create' => [
                            "caption" => "RS90 Device Instanz erstellen",
                            "moduleID" => "{3AD62E8A-68C2-BA18-223D-EF022958AE42}",
                            "location" => [],
                            "name" => $name,
                            "configuration" => [
                                "DeviceName" => $name ?? 'Unbekannt',
                                "DeviceID" => $did ?? 'Unbekannt',
                                "RemoteID" => $rid ?? 'Unbekannt'
                            ]
                        ]
                    ];
                }
            }
        }
        $this->SendDebug('DeviceTree rows', json_encode($deviceRows), 0);
        return $deviceRows;
    }

    private function LoadMacros(): array
    {
        $remoteIds = $this->GetJsonAttr('RemoteIDs', []);
        if (!is_array($remoteIds)) {
            $remoteIds = [];
        }

        // Vorhandene RS90 Macro Instanzen ermitteln
        $existingInstances = $this->FindExistingInstances("{223A81D9-D037-A052-A2E6-69F2057DAEB3}", "MacroID");

        $macroMap   = $this->GetJsonAttr('MacroListsByRemote', []);
        if (!is_array($macroMap))   $macroMap = [];

        $macroRows = [];
        foreach ($remoteIds as $rid) {
            $rid = (string)$rid;
            $remoteNodeId = $this->TreeId('remote:' . $rid);

            // Parent row (Remote)
            $macroRows[] = [
                'id'     => $remoteNodeId,
                'parent' => 0,
                'Node'   => $rid,
                'MacroID'     => '',
                'MacroName'   => ''
            ];

            $macros = $macroMap[$rid] ?? [];
            if (is_array($macros)) {
                foreach ($macros as $m) {
                    if (!is_array($m)) continue;
                    $mid  = (string)($m['id'] ?? '');
                    $name = (string)($m['name'] ?? '');
                    // $this->SendDebug(__FUNCTION__, "Macro $name mit MacroID $mid", 0);
                    $instanceID = 0;
                    if (isset($existingInstances[$mid])) {
                        $instanceID = $existingInstances[$mid];
                        $this->SendDebug(__FUNCTION__, "✅ Bereits verlinkte Instanz gefunden: $instanceID für MacroID $mid", 0);
                    }

                    $childId = $this->TreeId('macro:' . $rid . ':' . $mid);

                    $macroRows[] = [
                        'instanceID' => $instanceID,
                        'id'     => $childId,
                        'parent' => $remoteNodeId,
                        'Node'   => '',
                        'MacroID'     => $mid,
                        'MacroName'   => $name,
                        'create' => [
                            "caption" => "RS90 Macro Instanz erstellen",
                            "moduleID" => "{223A81D9-D037-A052-A2E6-69F2057DAEB3}",
                            "location" => [],
                            "name" => $name,
                            "configuration" => [
                                "MacroName" => $name ?? 'Unbekannt',
                                "MacroID" => $mid ?? 'Unbekannt',
                                "RemoteID" => $rid ?? 'Unbekannt'
                            ]
                        ]
                    ];
                }
            }
        }

        $this->SendDebug('MacroTree rows', json_encode($macroRows), 0);
        return $macroRows;
    }

    public function ReceiveData($JSONString): string
    {
        $payload_string = mb_convert_encoding($JSONString, 'ISO-8859-1', 'UTF-8');

        $this->SendDebug('Incoming', $payload_string, 0);

        $data = json_decode($payload_string, true);
        if (!is_array($data)) return '';

        $topic   = $data['Topic']   ?? '';
        $payload = $data['Payload'] ?? '';

        if (!is_string($topic) || !is_string($payload) || $topic === '') return '';

        $this->SendDebug('Topic', $topic, 0);
        $this->SendDebug('Payload', $payload, 0);

        // RemoteID aus Topic ziehen: Haptique/{RemoteID}/...
        $parts = explode('/', $topic);
        $remoteId = $parts[1] ?? '';
        $this->EnsureRemoteId($remoteId);

        // keys: Haptique/{RemoteID}/keys  payload like "button:10"
        if (($parts[2] ?? '') === 'keys') {
            $this->SetMapValue('LastKeyByRemote', $remoteId, $payload);
            return '';
        }

        // battery/status: Haptique/{RemoteID}/battery/status
        if (($parts[2] ?? '') === 'battery' && ($parts[3] ?? '') === 'status') {
            $lvl = (int)preg_replace('/[^0-9]/', '', $payload);
            $this->SetMapValue('BatteryByRemote', $remoteId, $lvl);
            return '';
        }

        // device/list (JSON Array): Haptique/{RemoteID}/device/list
        if (($parts[2] ?? '') === 'device' && ($parts[3] ?? '') === 'list') {
            $j = json_decode($payload, true);
            if (is_array($j)) {
                $this->SetMapValue('DeviceListsByRemote', $remoteId, $j);
            }
            return '';
        }

        // macro/list (JSON Array): Haptique/{RemoteID}/macro/list
        if (($parts[2] ?? '') === 'macro' && ($parts[3] ?? '') === 'list') {
            $j = json_decode($payload, true);
            if (is_array($j)) {
                $this->SetMapValue('MacroListsByRemote', $remoteId, $j);
            }
            return '';
        }

        return '';
    }

    public function GetConfigurationForm(): string
    {
        $this->SendDebug(__FUNCTION__, 'Konfigurator-Formular wird geladen', 0);

        $form = [
            "elements" => [
                [
                    'type' => 'RowLayout',
                    'items' => [
                        [
                            'type' => 'Image',
                            'name' => 'CantataLogo',
                            'image' => 'data:image/svg+xml;base64, ',
                            'width' => '250px', // passt die Breite der Grafik an
                        ],
                        [
                            'type' => 'Image',
                            'name' => 'CantataText',
                            'image' => 'data:image/svg+xml;base64, ',
                            'width' => '250px', // passt die Breite der Grafik an
                        ]
                    ]
                ],
                [
                    "type" => "ExpansionPanel",
                    "caption" => $this->Translate('RS90 Devices'),
                    "items" => [
                        [
                            "type" => "Configurator",
                            "name" => "RS90VDevices",
                            "caption" => $this->Translate('RS90 list'),
                            "delete" => true, // Löschen von Einträgen erlaubt
                            'sort' => [
                                'column' => 'RemoteID',
                                'direction' => 'ascending'
                            ],
                            'loadValuesFromConfiguration' => false, // Deaktiviert automatisches Laden
                            "rowCount" => 10, // Zeigt standardmäßig 10 Zeilen an
                            "columns" => [
                                [
                                    'caption' => $this->Translate('Remote ID'),
                                    'name' => 'RemoteID',
                                    'width' => 'auto',
                                    'visible' => true,
                                    'save' => false
                                ],
                                [
                                    'caption' => $this->Translate('Model'),
                                    'name' => 'Model',
                                    'width' => '200px',
                                    'visible' => true,
                                    'save' => false
                                ],
                                [
                                    'caption' => $this->Translate('Battery'),
                                    'name' => 'Battery',
                                    'width' => '200px',
                                    'visible' => true,
                                    'save' => false
                                ]
                            ],
                            "values" => $this->LoadRSDevices()
                        ]
                    ]
                ],
                [
                    "type" => "ExpansionPanel",
                    "caption" => $this->Translate('Device List'),
                    "items" => [
                        [
                            "type" => "Configurator",
                            "name" => "DevicesTree",
                            "caption" => $this->Translate('Device list'),
                            "delete" => true, // Löschen von Einträgen erlaubt
                            'sort' => [
                                'column' => 'DeviceName',
                                'direction' => 'ascending'
                            ],
                            'loadValuesFromConfiguration' => false, // Deaktiviert automatisches Laden
                            "rowCount" => 10, // Zeigt standardmäßig 10 Zeilen an
                            "columns" => [
                                [
                                    'caption' => $this->Translate('Remote ID'),
                                    'name' => 'Node',
                                    'width' => '400px',
                                    'visible' => true,
                                    'save' => false
                                ],
                                [
                                    'caption' => $this->Translate('Device Name'),
                                    'name' => 'DeviceName',
                                    'width' => 'auto',
                                    'visible' => true,
                                    'save' => false
                                ],
                                [
                                    'caption' => $this->Translate('Device ID'),
                                    'name' => 'DeviceID',
                                    'width' => '400px',
                                    'visible' => true,
                                    'save' => false
                                ]

                            ],
                            // "values" => []
                            "values" => $this->LoadDevices()
                        ]
                    ]
                ],
                [
                    "type" => "ExpansionPanel",
                    "caption" => $this->Translate('Macro list'),
                    "items" => [
                        [
                            "type" => "Configurator",
                            "name" => "MacroTree",
                            "caption" => $this->Translate('Macro list'),
                            "delete" => true, // Löschen von Einträgen erlaubt
                            'sort' => [
                                'column' => 'MacroName',
                                'direction' => 'ascending'
                            ],
                            'loadValuesFromConfiguration' => false, // Deaktiviert automatisches Laden
                            "rowCount" => 10, // Zeigt standardmäßig 10 Zeilen an
                            "columns" => [
                                [
                                    'caption' => $this->Translate('Remote ID'),
                                    'name' => 'Node',
                                    'width' => '400px',
                                    'visible' => true,
                                    'save' => false
                                ],
                                [
                                    'caption' => $this->Translate('Macro Name'),
                                    'name' => 'MacroName',
                                    'width' => 'auto',
                                    'visible' => true,
                                    'save' => false
                                ],
                                [
                                    'caption' => $this->Translate('Macro ID'),
                                    'name' => 'MacroID',
                                    'width' => '400px',
                                    'visible' => true,
                                    'save' => false
                                ]
                            ],
                            // "values" => []
                            "values" => $this->LoadMacros()
                        ]
                    ]
                ],
            ],
            'status' => [
                ['code' => IS_CREATING, 'icon' => 'inactive', 'caption' => $this->Translate('Device is being created')],
                ['code' => IS_ACTIVE, 'icon' => 'active', 'caption' => $this->Translate('Device connected and active')],
                ['code' => IS_DELETING, 'icon' => 'inactive', 'caption' => $this->Translate('Device is being deleted')],
                ['code' => IS_INACTIVE, 'icon' => 'inactive', 'caption' => $this->Translate('Device inactive')],
                ['code' => IS_NOTCREATED, 'icon' => 'error', 'caption' => $this->Translate('Device not created or error occurred')],
            ]
        ];
        $formSize = strlen(json_encode($form));  // Größe des Formulars in Bytes
        $this->SendDebug('Form Size', "Größe der Konfigurationsform: " . $formSize . " Bytes", 0);
        return json_encode($form);
    }
}