...
require_once __DIR__ . '/../libs/HaptiqueDebugHelper.php';
/**
...
class SSDPRoku extends IPSModule
{
    ...
    use HaptiqueDebugHelper, // Erweitert die SendDebug Methode von IPS um Arrays und Objekte.
    ...
    public function Create()
    {
        ...
        $this->RegisterTimer('SendNotify', 0, 'SSDPHaptique_TimerNotify($_IPS[\'TARGET\']);');
    ...
    public function ApplyChanges()
    {
        ...
        $this->RegisterHook('/hook/haptiqueemulator' . $this->InstanceID);
    ...
    {
        $this->SendDebug(__FUNCTION__, "Send Notification to " .self::ROKU_PORT, 0);
        $this->SendNotify(self::ROKU_PORT);
    }
        ...
        switch ($Request) { // REQUEST
            case 'M-SEARCH * HTTP/1.1':
                ...
                if (isset($Header['MAN']) && (strtolower($Header['MAN']) == '"ssdp:discover"')) {
                    ...
                    $this->SendSearchResponse($ReceiveData->ClientIP, $ReceiveData->ClientPort, self::ROKU_PORT);
                    return;
                }
                break;
