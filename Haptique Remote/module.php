...
class HaptiqueRemote extends IPSModule
{
    ...
    public function Create()
    {
        ...
        $this->RegisterVariableInteger('Volume', $this->Translate('Volume'), '~Intensity.100', 3);
        $this->RegisterVariableBoolean('LEDRingOnOff', $this->Translate('LED Ring On/Off'), '~Switch', 4);
        $this->RegisterVariableString('LEDRingColor', $this->Translate('LED Ring Color'), '', 5);
        $this->RegisterVariableBoolean('Backlight', $this->Translate('Backlight On/Off'), '~Switch', 6);
        $this->RegisterVariableString('LastKeyPress', $this->Translate('Last Key Press'), '', 7);
        $this->RegisterVariableString('ActiveSequence', $this->Translate('Active Sequence'), '', 8);
        $this->RegisterVariableInteger('Battery', $this->Translate('Battery'), '~Intensity.100', 9);
        $this->RegisterVariableString('RemoteStatus', $this->Translate('Remote Status'), '', 10);
    ...
    public function SetLEDRingColor(string $Value)
    {
        $this->SendDebug($this->Translate('SetLEDRingColor'), $this->Translate('LED Ring Color set to ') . $Value, 0);
        $this->SetValue('LEDRingColor', $Value);

        // (TODO) Per-color control is not exposed in the current RS90 MQTT API (only default animation toggle).
    }
    ...
    public function ReceiveData($JSONString)
    {
        ...
        if (($parts[2] ?? '') === 'keys') {
            ...
            // erwartetes Format: "button:9"
            if (preg_match('/button:(\d+)/', $payload, $m)) {
                $btn = (int)$m[1];

                $mapped = $this->mapButton($btn);

                // Wenn du die Nummer im String behalten willst:
                // $value = $mapped . ' (' . $btn . ')';
                $value = $mapped;

                $this->SendDebug('KeyButton', (string)$btn, 0);
                $this->SendDebug('KeyMapped', $value, 0);

                $this->SetValue('LastKeyPress', $value);
            } else {
                $this->SendDebug('KeyParseFailed', $payload, 0);
            }

            return '';
        }

        // battery_level: Haptique/{RemoteID}/battery_level  payload like "100"
        if (($parts[2] ?? '') === 'battery_level') {
            $lvl = (int)trim($payload);
            if ($lvl < 0)   $lvl = 0;
            if ($lvl > 100) $lvl = 100;

            $this->SendDebug('BatteryLevel', (string)$lvl, 0);
        ...
        if (($parts[2] ?? '') === 'status') {
            $status = trim((string)$payload);

            $this->SendDebug('RemoteStatus', $status, 0);
    ...
    public function GetConfigurationForm()
    {
        ...
        $form = [
            'elements' => [
                ...
                [
                    'type' => 'Label',
                    'caption' => $this->Translate('Remote Configuration')
                ],
                [
                ...
                [
                    'type' => 'Button',
                    'caption' => $this->Translate('Check Configuration'),
                    'onClick' => 'HaptiqueRemote_CheckConfiguration($id);'
                ],
                [
                    'type' => 'Button',
                    'caption' => $this->Translate('Execute Synchronization'),
                    'onClick' => 'HaptiqueRemote_SyncDevices($id);'
                ],
                [
                    'type' => 'Label',
