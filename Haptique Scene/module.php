...
class HaptiqueScene extends IPSModuleStrict
{
    ...
    public function ApplyChanges(): void
    {
        ...
        // Filter setzen basierend auf der SceneID
        $filter = sprintf('.*"SceneID":%d.*', $this->ReadPropertyInteger('SceneID'));
    ...
    {
        // Lese die Button-Konfiguration aus der Property
        $buttonConfig = json_decode($this->ReadPropertyString('ButtonConfig'), true);

        // Debug-Ausgabe für die gesamte Konfiguration
        $this->SendDebug(__FUNCTION__, "ButtonConfig: " . json_encode($buttonConfig), 0);

        // Suche nach dem passenden Button
        foreach ($buttonConfig as $btn) {
            if ($btn['Name'] === $button) {
                // Debug-Ausgabe, wenn der Button gefunden wurde
                $this->SendDebug(__FUNCTION__, "Button gefunden: " . json_encode($btn), 0);

                // Prüfen, ob eine gültige VariableID vorhanden ist
                if (isset($btn['VariableID']) && $btn['VariableID'] > 0) {
                    $value = isset($btn['Value']) ? $btn['Value'] : 0;

                    // Wert setzen
                    $this->SendDebug(__FUNCTION__, "Setze Wert: $value für Variable: " . $btn['VariableID'], 0);
                    RequestAction($btn['VariableID'], $value);
                } else {
        ...
        // Debug-Ausgabe, falls kein Button gefunden wurde
    ...
    public function UpdateLabelValue(string $Value, int $VariableID): void
    {
        ...
        // Text des Wertes aus dem Variablenprofil ermitteln
        $buttonConfig = json_decode($this->ReadPropertyString('ButtonConfig'), true);
        foreach ($buttonConfig as &$button) {
            if ($button['VariableID'] == $VariableID) {
                ...
                if ($profileName) {
                    ...
                    foreach ($profile['Associations'] as $association) {
                        if ($association['Value'] == $Value) {
                            $button['FormatedValue'] = $association['Name'];
                            break;
                        }
                    }
                }
        ...
        $this->UpdateFormField('FormatedValue', 'caption', $button['FormatedValue'] ?? '');
    }

    public function GetButtonsForm()
    {
        $buttons = $this->ReadAttributeString('Buttons');
        return $buttons;
    }
    ...
    public function GetConfigurationForm(): string
    {
        ...
        // Lade Werte aus der Property
        $this->SendDebug(__FUNCTION__, 'ButtonConfig: ' . $this->ReadPropertyString('ButtonConfig'), 0);
        $propertyConfig = json_decode($this->ReadPropertyString('ButtonConfig'), true);

        // Standardwerte direkt im Code definieren
        $defaultButtons = [
            ['Name' => 'Button •', 'VariableID' => 0, 'Value' => 0, 'ButtonLabel' => 'Erster Button'],
            ['Name' => 'Button ••', 'VariableID' => 0, 'Value' => 0, 'ButtonLabel' => 'Zweiter Button'],
            ['Name' => 'Button •••', 'VariableID' => 0, 'Value' => 0, 'ButtonLabel' => 'Dritter Button'],
        ];

        // Wenn Property-Werte vorhanden sind, diese mit den Standardwerten kombinieren
        $buttonConfig = $defaultButtons; // Start mit den Standardwerten
        if (!empty($propertyConfig)) {
            foreach ($propertyConfig as $key => $propertyButton) {
                if (isset($buttonConfig[$key])) {
                    // Überschreibe die editierbaren Werte (VariableID und Value) aus der Property
                    $buttonConfig[$key]['VariableID'] = $propertyButton['VariableID'];
                    $buttonConfig[$key]['Value'] = $propertyButton['Value'];

                    // Hole den Textwert aus dem Variablenprofil für die Spalte "Raw"
                    if ($propertyButton['VariableID'] > 0) {
                        $variable = IPS_GetVariable($propertyButton['VariableID']);
                        $profileName = $variable['VariableProfile'] ?? $variable['VariableCustomProfile'] ?? null;

                        if ($profileName) {
                            $profile = IPS_GetVariableProfile($profileName);
                            foreach ($profile['Associations'] as $association) {
                                if ($association['Value'] == $propertyButton['Value']) {
                                    $buttonConfig[$key]['FormatedValue'] = $association['Name'];
                                    break;
                                }
                            }
                        }
        ...
        $this->SendDebug(__FUNCTION__, 'Form Buttons: ' . json_encode($buttonConfig), 0);
        ...
        $form = [
            'elements' => [
                ...
                [
                    'type' => 'NumberSpinner',
                    'name' => 'SceneID',
                    'caption' => 'Scene ID', // Scene ID anzeigen
                    'enabled' => false // Nur lesbar machen
                ],
                [
                    'type' => 'List',
                    'name' => 'ButtonConfig',
                    'caption' => 'Button-Konfiguration',
                    'rowCount' => 3, // Maximale Zeilen
                    'add' => false,
                    'delete' => false,
                    'values' => $buttonConfig, // Vorbelegte Werte
                    'columns' => [
                        [
                            'caption' => 'Button Name',
                            'name' => 'Name',
                        ...
                        [
                            ...
                            'edit' => [
                                'type' => 'SelectVariable',
                                'onChange' => 'CRSXS_UpdateVariableID($id, $VariableID);',
                            ]
                        ],
                        [
                            'caption' => 'Wert',
                            ...
                            'edit' => [
                                ...
                                'onChange' => 'CRSXS_UpdateLabelValue($id, $Value, $VariableID);',
                            ]
                        ],
                        [
                            'caption' => 'Formated Value',
                            'name' => 'FormatedValue',
                            'width' => '150px'
                        ],
                        [
